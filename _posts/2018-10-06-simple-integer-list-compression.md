...
I have been playing around with compression algorithms since a while now. I have tried many different things, from well-known techniques to new algorithms. I have failed plenty of times when having a go at new things. But, I have always got something valuable out of it.

One of the most important (yet obvious) things I have learned is that any compressed data must be reversible in some way. Otherwise, it would be useless as you could not get the original input from the compressed output.

Sometimes, you are too eager to try that fantastic and world-changing compression algorithm you just came up with that you do not pay much attention to the details. You might overlook the decompression process and make wrong assumptions about it. And you might end up having an amazing compression ratio and a pointless bunch of bits at the same time.
...
You decide you want to see first how it looks like in binary:

*01111010, 00001110, 00001001, 11101001, 00111011, 00101100, 11100011*

"Well, that is 56 bits for 7 items", you think.

Then, you wonder: "How could I compress that bunch of bits? Mmm... I need to basically represent those numbers with fewer bits. What if I get rid of the leading zeros? Let's see how the output would be in binary".

*1111010, 1110, 1001, 11101001, 111011, 101100, 11100011*

"Amazing! 43 bits only! That is 23% smaller than the original list!".

You are very excited and you rush to implement the algorithm, create some tests, measure the performance, and so on. Everything is going so well that you get that feeling of "I must have made a huge mistake". And you are right. You completely forgot the decompression.

So, how could anyone decompress that stream of bits? Be aware that I added some commas to improve readability. In the "real world", a compressed chunk is just a sequence of bits with no breaks between elements. Therefore, it is impossible to recover the original information as there is no rule that tells you when an element starts and when it ends.
...
The decompression algorithm would be something like the following:
...
4. It continues the same way until it reaches the end of the list. The remaining values **<span style="color:lightsalmon">55</span>, <span style="color:tomato">30</span>, <span style="color:lightsalmon">25</span>, <span style="color:tomato">9</span>, <span style="color:lightsalmon">3</span>** are coded with 7, 6, 5, 5, 4 bits respectively.
...
As you can see, the output is smaller than the input, so we have achieved some compression. Concretely, the plain list is 64 bits long whereas the compressed one takes up 50 bits, roughly a 22% smaller.
...
Even if the list has an ascending order, you will still need to encode the values in descending order. A solution would be just to add a flag that indicates the actual order of the list at the beginning of the compressed stream. One bit would be enough: 0 for ascending and 1 for descending.
...
* It must be really fast when compressing and decompressing (pending to test).
* It will always produce a smaller or equal output than the input (headers apart).
* The compression ratio might be better than other algorithms in some kind of data distributions like a short list of sparse integers (pending to test).
...
Although I have made some assumptions about the strengths of this algorithm, and they need to be proved, I am really confident of its **simplicity and speed**. Both compression and decompression algorithms are just a few lines long with no complicated calculations on it. Plus, considering that the complexity of $$minBitsLen$$ is constant, the algorithm run in a linear time with a complexity of $$O(n)$$, so it must be really quick.

Again, these are all gut feelings as I have not tested anything yet. I would like to come back to this algorithm and dig deeper into it. More posts will come after this one!
